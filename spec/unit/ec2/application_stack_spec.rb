...
describe ApplicationStack = Awsymandias::EC2::ApplicationStack do
  ...
  describe "instances" do
    it "should be empty by default" do
      ApplicationStack.new("foo").instances.should be_empty
    end

    it "should be settable through the initializer" do
      stack = ApplicationStack.new("foo", :instances => { :app1 => {} })
      stack.instances[:app1].should == {}
    end
  end

  describe "instance" do
    it "should allow the definition of a basic, empty instance" do
    ...
    it "should use the parameters given to the instance definition" do
      stack = ApplicationStack.new("foo") do |s|
        s.instance :app1, :foo => "bar"
      end
      stack.instances[:app1].should == { :foo => "bar" }
    end

    it "should allow for the creation of multiple instances" do
      stack = ApplicationStack.new("foo") do |s|
        s.instance :app1, :foo => "bar"
        s.instance :app2, :foo => "baz"
      end
      stack.instances[:app1].should == { :foo => "bar" }
      stack.instances[:app2].should == { :foo => "baz" }
    end

    it "should map multiple instances to the same set of parameters" do
      stack = ApplicationStack.new("foo") do |s|
        s.instance :app1, :app2, :foo => "bar"
      end
      stack.instances[:app1].should == { :foo => "bar" }
      stack.instances[:app2].should == { :foo => "bar" }
    end
  ...
  describe "launch" do
    it "should launch its instances when launched" do
      s = ApplicationStack.new("test") do |s| 
        s.instance "db1",  :instance_type => Awsymandias::EC2::InstanceTypes::C1_XLARGE
        s.instance "app1", :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE
      end
  ...
  describe "launched?" do    
    it "should be false initially" do
      s = ApplicationStack.new("test") {|s| s.instance "db1", :instance_type => Awsymandias::EC2::InstanceTypes::M1_LARGE}
      s.should_not be_launched
    end

    it "should be true if launched and instances are non-empty" do
      s = ApplicationStack.new("test") { |s| s.instance "db1" }
  ...
  describe "running?" do
    it "should be false initially" do
      ApplicationStack.new("test") {|s| s.instance "db1"}.should_not be_running
    end
    ...
    it "should be false if launched but all instances are pended" do
      ...
      ApplicationStack.new("test") {|s| s.instance "db1"}.launch.should_not be_running
    end
